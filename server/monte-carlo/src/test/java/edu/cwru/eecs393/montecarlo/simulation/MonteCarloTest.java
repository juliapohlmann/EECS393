...
public class MonteCarloTest extends TestCase{
  
  public void testDrift(){
    Stock s = new Stock("AAPL", 23.40, 20.05);
    Stock portfolio[] = {s};
    MonteCarlo m = new MonteCarlo(5,10,50,portfolio);
    double aveAnnualReturn = 10;
    double variance = 5;
    assert(m.drift(aveAnnualReturn,variance) == (aveAnnualReturn - variance/2));
  }
  ...
  public void testNextYearPrice(){
    ...
    assert(m.nextYearPrice(currentPrice, drift, randomValue) == (currentPrice * (Math.exp(drift + randomValue))));
  }
  ...
  public void testPeriodicAnnualReturn(){
    ...
    assert(m.periodicAnnualReturn(currentPrice, previousPrice) == Math.log(currentPrice/previousPrice));
  }
