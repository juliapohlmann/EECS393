...
public class MonteCarloSimulation implements Simulation {
 ...
 // simulation
  ...
  public MonteCarloSimulation(SimulationParameters simParameters, Map<String,FinancialData> financialData){
    ...
    for(FinancialData fd: financialData.values()){
      portfolio.add(new Stock(fd.getTicker(), fd.getBid(), fd.getHistoricalData().first(), simParameters.getTickerToAllocation().get(fd.getTicker())));
    }
 ...
 public SimulationType getSimulationType() {
  ...
  public void forecastAllStocks(){
    ...
    for(Iterator<E> iter = portfolio.iterator(); iter.hasNext(); ){
      Stock s = iter.next();
  ...
  public double[] forecast(Stock stock){
    ...
    for(int i = 1; i <= duration; i++){     
      double randomValue = randomValue(stdDev);
  ...
  public double randomValue(double stdDev){
