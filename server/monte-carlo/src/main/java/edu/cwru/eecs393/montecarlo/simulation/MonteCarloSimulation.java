...
public class MonteCarloSimulation implements Simulation {
	...
	// TODO fields and constructor with everything that is needed for this simulation
	...
	public double[] forecast(Stock stock) {
		...
		//will be expanded in later versions to include up to 5 years of historic prices
		...
		double forecastedPrice[] = new double[duration + 1];  //duration + 1 to include price at time 0
		forecastedPrice[0] = currentPrice;  //year 0 price is current price

		/*for each year of duration, calculate the price.
		 * Model is based on the drift of annual returns and a random variable
		 * Assumes stocks follow a "Random Walk" and employs random, Brownian Motion
		 */
	...
	//inverse probability a random value occurs on a normal distribution scaled by the standard deviation
